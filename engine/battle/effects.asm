...
FreezeBurnParalyzeEffect:
    ...
    ; (lado do jogador atacando)
    ld a, [wEnemyMonStatus]
    and a
    jp nz, CheckDefrost ; alvo já statusado
    ld a, [wPlayerMoveType]
    ld b, a
    ld a, [wEnemyMonType1]
    cp b
    ret z  ; same type => sem efeito (ex.: Ice move em Ice-type)
    ld a, [wEnemyMonType2]
    cp b
    ret z

    ; Checar side effect ID para decidir se é burn, freeze ou paralyze
    ld a, [wPlayerMoveEffect]
...
TriAttackEffect:
    ...
    ldh a, [hWhoseTurn]
    and a
    jr nz, .enemyUsingTriAttack

    ; Checar se o alvo (enemy) já tem status
    ld a, [wEnemyMonStatus]
    and a
    ret nz

    ; chance total de 20% (por exemplo)
    ld c, 51  ; ~20% (51/256)
    call BattleRandom
    cp c
    ret nc    ; se random >= 51, sem status

    ; Se passar, aleatório 1/3
    call BattleRandom
    cp 85
    jr c, .burn1
    cp 170
    jr c, .freeze1
    ; se não cair em burn ou freeze => paralyze
...
.enemyUsingTriAttack:
    ; lógica semelhante pro lado do jogador
